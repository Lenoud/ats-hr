...
为避免本地反复启动后留下脏注册或制造更多冲突，当前实现增加了两层保护：

- `run-services.sh` 在真正启动前会检查目标端口是否已被占用，若 `8081`、`8082`、`8083`、`9090`、`9091` 或启用 gateway 时的 `8080` 已被占用，会直接失败退出
- 各服务在向 Consul 注册前，会先清理“相同 service name + address + port”的旧实例注册，再注册当前进程对应的新实例

这两项保护只用于保持本地开发环境干净，不改变正式运行时的服务发现模型。
