...
|------|------|-----------|-----------|------|
| API Gateway | 编排首页、健康聚合、基于路径前缀的反向代理与 Consul HTTP 服务发现 | 8080 | - | ✅ 已实现 |
...
- `gateway` 当前是轻量级路径代理，按路径前缀决定目标服务，并通过“逻辑服务名 + HTTP 协议”动态解析可用实例地址；其首页同时提供主链路调试编排能力，但不引入额外 orchestration backend。
